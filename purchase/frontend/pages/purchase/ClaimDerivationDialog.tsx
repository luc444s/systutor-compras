...
type ClaimDerivationDialogProps = {
  open: boolean;
  orderId: string | null;
...
export function ClaimDerivationDialog({ open, orderId, setError, onClose }: ClaimDerivationDialogProps) {
  ...
  return (
    <Dialog open={open} title="Derivación de reclamos" description="Genera reclamaciones desde las diferencias de la orden." onClose={onClose} maxWidthClassName="max-w-3xl">
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          {result ? <span className="text-xs text-muted-foreground">Creadas {result.created.length} · Omitidas {result.skipped}</span> : null}
        </div>
        {result ? (
